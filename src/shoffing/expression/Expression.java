...
public class Expression
{
	...
	{
		expression = new BinaryTree<ExpressionPart>(new Constant((double) (Math.random() * (maxVal - minVal) + minVal)));
	...
	public static Expression mutate(Expression exp, double constChange)
	{
		...
		for(int i = 0; i < result.expression.size(); i++)
		{
			...
			{
				double randChange = round2((double) (Math.abs(constChange) * (2 * Math.random() - 1)), decimalPlaces);
	...
	public void expand()
	{
		...
		{
			// rhs is constant
			double rhsVal = MIN_VAL + (Math.random()) * (MAX_VAL - MIN_VAL);
			...
			switch(ct.getRoot().getType())
			{
				case OPERATOR_ADD:
					if(ct.getLeft().getRoot().getType() == ExpressionPart.Type.OPERATOR_SUB)
					{
						if( ct.getLeft().getRight().equals(ct.getRight()) )
							expression.setTreeAt(i, new BinaryTree<ExpressionPart>(ct.getLeft().getLeft()));	// (N - E) + E, ROOT = ROOT.LEFT.LEFT
					} else if(ct.getRight().getRoot().getType() == ExpressionPart.Type.OPERATOR_SUB) {
						if( ct.getLeft().equals(ct.getRight().getRight()) )
							expression.setTreeAt(i, new BinaryTree<ExpressionPart>(ct.getRight().getLeft()));	// E + (N - E), ROOT = ROOT.RIGHT.LEFT
						...
						expression.setTreeAt(i, newTree);	// E + E, ROOT = TREE(MULT, 2, ROOT.RIGHT)
				...
				case OPERATOR_SUB:
					if(ct.getLeft().getRoot().getType() == ExpressionPart.Type.OPERATOR_ADD)
					{
						if( ct.getLeft().getLeft().equals(ct.getRight()) )
							expression.setTreeAt(i, new BinaryTree<ExpressionPart>(ct.getLeft().getRight()));	// (N + E) - E, ROOT = ROOT.LEFT.RIGHT
						else if( ct.getLeft().getRight().equals(ct.getRight()) )
							expression.setTreeAt(i, new BinaryTree<ExpressionPart>(ct.getLeft().getLeft()));	// (E + N) - E, ROOT = ROOT.LEFT.LEFT
					} else if( ct.getLeft().equals(ct.getRight()) ) {
						expression.setTreeAt(i, new BinaryTree<ExpressionPart>(new Constant(0)));				// E - E, ROOT = 0
					} else if(ct.getLeft().getRoot().getType() == ExpressionPart.Type.CONSTANT && ct.getRight().getRoot().getType() == ExpressionPart.Type.CONSTANT) {
						expression.setTreeAt(i, new BinaryTree<ExpressionPart>(new Constant(((Constant) ct.getLeft().getRoot()).getValue() - ((Constant) ct.getRight().getRoot()).getValue())));
					}
					break;
				case OPERATOR_MULT:
					if(ct.getLeft().getRoot().getType() == ExpressionPart.Type.OPERATOR_DIV)
					{
						...
						expression.setTreeAt(i, newTree);	// E * E, ROOT = TREE(POW, ROOT.LEFT, 2)
					} else if(ct.getLeft().getRoot().getType() == ExpressionPart.Type.CONSTANT && ct.getRight().getRoot().getType() == ExpressionPart.Type.CONSTANT) {
						expression.setTreeAt(i, new BinaryTree<ExpressionPart>(new Constant(((Constant) ct.getLeft().getRoot()).getValue() * ((Constant) ct.getRight().getRoot()).getValue())));
					}
					break;
	...
	{
		if(places >= 0)
			return Math.round(val * Math.pow(10, places)) / (double) Math.pow(10, places);
		else
			return val;
	}
